function [samples,Weight] = Construct_Samples(n,m,rate,sampling_mode,symmetric_return)
...
elseif sampling_mode == 1
    L = n*(n-1)/2;
    samps = ceil(L*rate);
%     inds = randsample(L,samps,false); %Sampling without replacement
    inds = randsample(L,samps,true); %Sampling with replacement
